...
std::string Unicode_String_to_UTF8_std_string(const uint16_t * pwcs, int maxlen)
{
	static uint8_t  s[8*JFS_PATH_MAX];
 	maxlen=8*JFS_PATH_MAX-1;
	memset(s, 0, sizeof (s));
	int extra_bytes=0;
	const uint16_t *ip;
	uint8_t *op;
...
std::vector<UniChar> string_to_vector(const std::string& s){
